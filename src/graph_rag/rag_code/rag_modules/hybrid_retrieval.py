...
class HybridRetrievalModule:
    ...
    def __init__(self, config, vector_module, data_module, llm_client):
        self.config = config
        # 将传入的 index_module (可能是 Milvus, Annoy 或 FAISS) 统一存储
        self.vector_db_module = vector_module 
    ...
    def vector_search_enhanced(self, query: str, top_k: int = 5) -> List[Document]:
        """增强的向量检索：无论底层是哪个库，调用接口都一致"""
        try:
            # 统一调用接口：similarity_search
            vector_results = self.vector_db_module.similarity_search(query, k=top_k*2)
