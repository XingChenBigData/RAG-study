...
class GraphRAGRetrieval:
    ...
    def graph_rag_search(self, query: str, top_k: int = 5) -> List[Document]:
        ...
        try:
            ...
            # 3. 如果图RAG没有结果，尝试简单的Neo4j查询作为降级
            if not results:
                logger.info("图RAG无结果，尝试简单Neo4j查询...")
                results = self._fallback_neo4j_search(query, top_k)
            ...
            logger.error(f"图RAG检索失败: {e}")
            # 降级搜索
            return self._fallback_neo4j_search(query, top_k)

    def _fallback_neo4j_search(self, query: str, top_k: int) -> List[Document]:
        """降级Neo4j搜索"""
        results = []
        ...
        try:
            with self.driver.session() as session:
                ...
                RETURN r.nodeId as node_id, r.name as name, 
                    r.description as description, r.category as category
                LIMIT $limit
                """
                
                # 提取查询中的关键词
                keywords = ["豆腐", "肉类", "口感", "替代"]
                for keyword in keywords:
                    if keyword in query:
                        ...
                        for record in result:
                            content = f"菜品: {record['name']}\n"
                            if record['description']:
                                content += f"描述: {record['description']}\n"
                            if record['category']:
                                content += f"分类: {record['category']}"
                            
                            doc = Document(
                                page_content=content,
                                metadata={
                                    "search_type": "fallback_neo4j",
                                    "node_id": record["node_id"],
